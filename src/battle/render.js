...
export function drawMinimap(ctx, frame, rect, view) {
  ...
  // 移动端镜头一次只覆盖局部战场，在小地图保留更克制的雷达扫线与回波，
  // 让全图被动仍可读；它仍只来自己方私有 radar 字段。
  if (!spectating && state.phase !== "finished" && frame.radar?.active && ownTeam?.ships?.main?.alive) {
    const radar = frame.radar;
    const source = ownTeam.ships.main;
    const sourceX = rect.x + (source.x / LOGICAL) * rect.width;
    const sourceY = rect.y + (source.y / LOGICAL) * rect.height;
    const angle = radarAngleAt(radar, Number(state.elapsed) || 0);
    ...
    for (const contact of radar.contacts || []) {
      ...
      const alpha = radarContactAlpha(contact, Number(state.elapsed) || 0);
      if (alpha <= 0) continue;
