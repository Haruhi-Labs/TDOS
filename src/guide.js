// ═══════════════════════════════════════════════════════════════
// 玩法说明（路由 /guide）
// 目标：实用、易上手。先「三步开打」给最小可玩集，再分要点 + 分端操作 + 贴士。
// ═══════════════════════════════════════════════════════════════
...
const QUICKSTART = [
  ...
  "<b>下达航线</b>：选中一艘舰，给它指一条航线，舰队就会自动航行过去；推进功率决定快慢与转弯灵活度。",
  "<b>自动交火</b>：敌人进入射程、且被你任一单位看见时，舰炮会<b>自动开火</b>——你专心走位、放技能就好。",
];
...
const SECTIONS = [
  ...
  {
    title: "分离游走",
    body: "一级 / 二级分离让副舰脱队独立行动：转向更灵活、能量各自结算。适合包抄、逼对方分火、单独偷点。编队一起走时，航速取队内<b>最慢</b>的那艘。",
  },
  {
    title: "难度（仅单人）",
