...
def snapshot(**overrides: int | float | None) -> ResourceSnapshot:
...
def test_each_pressure_source_blocks_training() -> None:
    ...
    decision = evaluate_resource_safety(
        snapshot(
            ...
            memory_free_percent=thresholds.minimum_memory_free_percent - 1,
...
def test_real_snapshot_is_read_only_and_well_formed() -> None:
    ...
    assert current.memory_free_percent is None or 0 <= current.memory_free_percent <= 100
